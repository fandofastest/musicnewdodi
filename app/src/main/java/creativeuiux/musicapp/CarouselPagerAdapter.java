...
public class CarouselPagerAdapter extends FragmentPagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    public CarouselPagerAdapter(AlbumListActivity context, FragmentManager fm) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        try {
            if (position == AlbumListActivity.FIRST_PAGE)
                scale = BIG_SCALE;
            else
                scale = SMALL_SCALE;

            position = position % AlbumListActivity.count;
    ...
    public int getCount() {
        int count = 0;
        try {
            count = AlbumListActivity.count * AlbumListActivity.LOOPS;
        } catch (Exception e) {
